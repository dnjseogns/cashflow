...
const Baby = ({children}) => {
    ...
    return (
        ...
        <h4 className='title'>요약</h4>
            <p>
                A : 28~55세(초봉2900만,저축40%이상) → 34세(결혼 맞벌이, 전세3억, 아반떼구매) → 40세(집매매6억) → 56~65세(재취업,기존연봉60%) → 65세(국민연금수령)
                <br/>B : 28~55세(초봉2900만,저축40%이상) → 34세(결혼 맞벌이, 전세3억, 아반떼구매) → 36세(아이 1명 출산) → 40세(집매매6억) → 56~65세(재취업,기존연봉60%) → 65세(국민연금수령)
                <br/>C : 28~55세(초봉2900만,저축40%이상) → 34세(결혼 맞벌이, 전세3억, 아반떼구매) → 36/38세(아이 2명 출산) → 40세(집매매6억) → 56~65세(재취업,기존연봉60%) → 65세(국민연금수령)
            </p>
            ...
            <p>
                ...
                <br/><span style={{marginLeft:"75px"}}>저출생의 원인으로 아이를 안낳는 것도 있지만, 둘째를 낳지 않는 것도 원인 중 하나라고 합니다. 만약 위 그래프대로라면, 둘째를 가지기는 쉽지 않을 것 같습니다.</span>
            </p>
        </div>
        </Fragment>
    )
